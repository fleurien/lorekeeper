    ...
    */
    ...
    public function createPetDrop($data)
    {
        ...
        try {
            ...
            $data['data']['drop_name'] = $data['drop_name'] ?? null;
    ...
    public function updatePetDrop($drop, $data)
    {
        ...
        try {
            ...
            if (PetDropData::where('pet_id', $data['pet_id'])->where('id', '!=', $drop->id)->exists()) {
                throw new \Exception('This pet already has drop data. Consider editing the existing data instead.');
            }
            ...
            if (isset($data['item_id']) && $data['item_id']) {
                foreach ($data['item_id'] as $key=>$itemData) {
                    foreach ($itemData as $param=>$itemId) {
                        if (isset($itemId) && $itemId) {
                            $item = Item::find($itemId);
                            if (!$item) {
                                throw new \Exception('One or more of the items selected are invalid.');
                            }
                            ...
                            if (!$minQuantity && !$maxQuantity) {
                                throw new \Exception('One or more of the items does not have either a minimum or maximum quantity.');
                            }
                            if (!$minQuantity || !$maxQuantity) {
                                if ($minQuantity && !$maxQuantity) {
