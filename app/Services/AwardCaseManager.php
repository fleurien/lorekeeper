    ...
    public function transferStack($sender, $recipient, $stacks, $quantities)
    {
        ...
        try {
            foreach ($stacks as $key=>$stack) {
                $quantity = $quantities[$key];
                if (!$sender->hasAlias) {
                    throw new \Exception('Transfer failed: Your alias must be verified before you can perform this action.');
                }
                if (!$stack) {
        ...
        try {
            if ($owner->logType == 'User') {
                foreach ($stacks as $key=>$stack) {
                    ...
                    $quantity = $quantities[$key];
                    if (!$owner->hasAlias) {
                        throw new \Exception('Your alias account must be verified before you can perform this action.');
                    }
                    if (!$stack) {
